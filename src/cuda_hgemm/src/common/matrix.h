...
public:
  ...
  void csrToBcsr() {
    // First prepare the info arrays - adjusted for BLOCKSIZE
    size_t numColRegions =
        (m_col + MMA_K * BLOCKSIZE - 1) / (MMA_K * BLOCKSIZE);
    size_t numRowRegions =
        (m_row + MMA_M * BLOCKSIZE - 1) / (MMA_M * BLOCKSIZE);
    ...
    for (size_t row = 0; row < m_row; row++) {
      for (size_t j = csrRowPtr_host[row]; j < csrRowPtr_host[row + 1]; j++) {
        size_t col = csrColIdx_host[j];
        size_t rowRegion = row / (MMA_M * BLOCKSIZE);
        size_t colRegion = col / (MMA_K * BLOCKSIZE);
    ...
    // Create mapping for nonzero blocks
    ...
    // Allocate BCSR arrays
    bcsrRowPtr_host =
        (int *)calloc(sizeof(int), (m_row / (MMA_M * BLOCKSIZE) + 1));
    ...
    // Fill rowPtr array
    size_t num_blocks = 0;
    for (size_t row = 0; row < m_row; row += MMA_M * BLOCKSIZE) {
      bcsrRowPtr_host[row / (MMA_M * BLOCKSIZE)] = num_blocks;

      // Find unique column regions in this row block
      std::set<size_t> colRegionsInThisRow;

      for (size_t localRow = row;
           localRow < std::min(row + MMA_M * BLOCKSIZE, m_row); localRow++) {
        for (size_t j = csrRowPtr_host[localRow];
             j < csrRowPtr_host[localRow + 1]; j++) {
          size_t col = csrColIdx_host[j];
          size_t colRegion = col / (MMA_K * BLOCKSIZE);
          colRegionsInThisRow.insert(colRegion);
        }
      }

      // Fill colIdx for this row block
      for (size_t colRegion : colRegionsInThisRow) {
        size_t blockIndex =
            (row / (MMA_M * BLOCKSIZE)) * numColRegions + colRegion;
        if (blockInfo_host[blockIndex] != 0) {
          bcsrColIdx_host[num_blocks] = colRegion * (MMA_K * BLOCKSIZE);
          num_blocks++;
        }
      }
    }
    bcsrRowPtr_host[m_row / (MMA_M * BLOCKSIZE)] = num_blocks;
    ...
    for (size_t row = 0; row < m_row; row++) {
      for (size_t j = csrRowPtr_host[row]; j < csrRowPtr_host[row + 1]; j++) {
        ...
        // Calculate position within block structure
        size_t tileY = (row % (MMA_M * BLOCKSIZE)) / MMA_M;
        size_t tileX = (col % (MMA_K * BLOCKSIZE)) / MMA_K;
        size_t innerRow = row % MMA_M;
        size_t innerCol = col % MMA_K;

        size_t offset = tileY * MMA_K * MMA_M * BLOCKSIZE +
                        tileX * MMA_M * MMA_K + innerRow * MMA_K + innerCol;

        size_t bcsrIndex = relativeBlockIndexMapping_host[blockIndex] * MMA_M *
                               MMA_K * BLOCKSIZE * BLOCKSIZE +
                           offset;
  ...
  //       // Calculate position within BLOCKSIZE x BLOCKSIZE grid
  //       size_t tileY = (row % (MMA_M * BLOCKSIZE)) / MMA_M;
  //       size_t tileX = (col % (MMA_K * BLOCKSIZE)) / MMA_K;

  //       // Calculate position within MMA tile
  //       size_t innerRow = row % MMA_M;
  //       size_t innerCol = col % MMA_K;
  ...
  //   bcsrRowPtr_host[m_row / MMA_M] = num_blocks; // Update last entry of
  //   rowPtr
  ...
  //       // relativeBlockIndexMapping[blockIndex], blockSize, offset,
  //       // bcsrIndex);
  //       bcsrVal_host[bcsrIndex] = val;
  //     }
  //   }
  ...
  void csrToBcsrKnapsacking() {
    ...
    for (size_t row = 0; row < m_row; row += MMA_M) {
      ...
      printf("%lu %lu %f %ld    ==>\n", std::get<0>(a), std::get<1>(a),
      std::get<2>(a), std::get<3>(a)); printf("span %lu addLeft %lu addRight %lu
      toAdd %lu ", span, potentialAddLeft, potentialAddRight, to_add); printf("
      start  %lu end %lu \n", start, end); */
    ...
    for (size_t rowRegion = 0; rowRegion < startsEndsOfCols.size();
      ...
      for (size_t element = 0; element < startsEndsOfCols[rowRegion].size();
        ...
        // relBlock, relColumn, relBlock * MMA_M * MMA_K + (row % MMA_M) * MMA_K
        // + relColumn);
      }
      // printf("\n");
    }
