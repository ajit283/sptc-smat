...
public:
  ...
  void csrToBcsr() {
    ...
    for (size_t row = 0; row < m_row; row++) {
      for (size_t j = csrRowPtr_host[row]; j < csrRowPtr_host[row + 1]; j++) {
        ...
        // size_t offset = ((row % MMA_M) / 2 + ((col % MMA_K) / 8) * 8) * MMA_K
        // +
        //                 (row % MMA_M) % 2;
        size_t bcsrIndex =
  ...
  void filterBcsrBlocks() {
    ...
    int numColRegions = (m_col + MMA_K - 1) / MMA_K;
    ...
    sparseBcsrVal_host =
        (half *)calloc(totalSparseBlocks * MMA_M * MMA_K, sizeof(half));
    ...
    denseBcsrVal_host =
        (half *)calloc(totalDenseBlocks * MMA_M * MMA_K, sizeof(half));
    ...
    for (int r = 0; r < numRowRegions; r++) {
      for (int c = 0; c < numColRegions; c++) {
        ...
        if (blockInfo_host[global_index] == 1) { // 2:4 sparse block.
          ...
          memcpy(sparseBcsrVal_host + idx * MMA_M * MMA_K,
                 bcsrVal_host + relIndex * MMA_M * MMA_K,
                 MMA_M * MMA_K * sizeof(half));
          sparseRowCounter[r]++;
          ...
          memcpy(denseBcsrVal_host + idx * MMA_M * MMA_K,
                 bcsrVal_host + relIndex * MMA_M * MMA_K,
                 MMA_M * MMA_K * sizeof(half));
          denseRowCounter[r]++;
        }
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaMalloc((void **)&sparseBcsrVal_dev,
                   totalSparseBlocks * MMA_M * MMA_K * sizeof(half)));
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaMalloc((void **)&denseBcsrVal_dev,
                   totalDenseBlocks * MMA_M * MMA_K * sizeof(half)));
...
private:
  ...
  // New members for filtered 2:4 sparse blocks
