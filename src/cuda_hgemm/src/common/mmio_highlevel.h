...

{
  ...
  half *csrVal_tmp = (half *)malloc(nnz_mtx_report * sizeof(half));
...
int mmio_data(int *csrRowPtr, int *csrColIdx, half *csrVal, char *filename)
...
int mmio_allinone(size_t *m, size_t *n, size_t *nnz, int *isSymmetric,
  ...
  half *csrVal_alias = (half *)malloc(nnz_tmp * sizeof(half));
  ...
  if (isSymmetric_tmp) {
    for (int i = 0; i < nnz_mtx_report; i++) {
      if (csrRowIdx_tmp[i] != csrColIdx_tmp[i]) {
        int offset = csrRowPtr_alias[csrRowIdx_tmp[i]] +
                     csrRowPtr_counter[csrRowIdx_tmp[i]];
