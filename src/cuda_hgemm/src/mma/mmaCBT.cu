...
__global__ void mmaCBTKernelSparse(half *bcsrValuesA, int *bcsrRowPtrA,
...
#pragma unroll
  for (size_t ptr = bcsrRowPtrA[blockRow]; ptr < bcsrRowPtrA[blockRow + 1];
    ...
    cooperative_groups::memcpy_async(
        group, &A_smem[0][0], &bcsrValuesA[relativeIndex * MMA_M * MMA_K],
        A_size);
...
void mmaCBTKernel(half *bcsrValuesA, int *bcsrRowPtrA, int *bcsrColIdxA,
  ...
  mmaCBTKernelSparse<<<grid, block>>>(bcsrValuesA, bcsrRowPtrA, bcsrColIdxA, B,
