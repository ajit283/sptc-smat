...
#pragma unroll
  for (size_t i = 0; i < K_tiles; ++i) {
    ...
    if (sparsityInfo == 2) {
      long4 *src = // length 16
          ((long4
                *)(&bcsrValuesA[(relativeIndex)*MMA_M * MMA_K * BLOCK * BLOCK +
                                warp_id * MMA_K * MMA_M +
                                (lane_id / 2) * MMA_K]) +
           lane_id % 2);
      ...
      *(((long4 *)(sparseMatrixA)) +
        ...
        warp_id * MMA_M * (MMA_K / 16) + lane_id) = *src;
    }
...
#pragma unroll
      ...
      *((int4 *)(((long4 *)(sparseMatrixA)) +
                 relativeIndex * MMA_M * (MMA_K / 16) * BLOCK * BLOCK +
                 warp_id * MMA_M * (MMA_K / 16)) +
  ...
  auto loadStages = [&] __device__(size_t stage_ptr, int stage) {
    if (stage_ptr < bcsrRowPtrA[blockRow + 1]) {
      ...
      if (sparsityInfo == 2) {
        ...
        // cuda::memcpy_async(
        //     (((long4 *)(&A_smem[stage][warp_id_y][warp_id_x][lane_id /
        //     2][0])) +
        //      lane_id % 2),
        //     &zeros, sizeof(long4), pipe);

        // For matrix B
        if (lane_id < MMA_N * 2) { // Original condition preserved
          cuda::memcpy_async(
              ((long4
                    *)(&B_smem[stage][warp_id_y][warp_id_x][(lane_id / 2)][0]) +
               lane_id % 2),
  ...
  for (size_t ptr = bcsrRowPtrA[blockRow]; ptr < bcsrRowPtrA[blockRow + 1];
    ...
    for (int j = 0; j < BLOCK; j++) {
      ...
      if (sparsityInfo == 2) {
        ...
        A_smem_lane_addr =
            __cvta_generic_to_shared(&A_smem[stage][warp_id_y][j][lane_id % 16]
                                            [(lane_id / 16) * 8 + 16]);
        LDMATRIX_X4(RA[stage][0], RA[stage][1], RA[stage][2], RA[stage][3],
                    A_smem_lane_addr);

        B_smem_lane_addr =
            __cvta_generic_to_shared(&B_smem[stage][warp_id_y][j][lane_id % 8]
                                            [((lane_id / 8) % 2) * 8 + 16]);
        LDMATRIX_X2(RB[stage][0], RB[stage][1], B_smem_lane_addr);
        ...
        metadata_local[2] = (char)((
            Meta_smem_sparse[stage][warp_id_y][warp_id_x][(lane_id / 4) + 8]
                            [0 + 2 * (lane_id % 2)]));
